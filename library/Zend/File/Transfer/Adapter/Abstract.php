...
abstract class Zend_File_Transfer_Adapter_Abstract
{
    ...
    public function isValid($files = null)
    {
        ...
        foreach($check as $key => $content) {
            if (array_key_exists_wrapper('validators', $content) &&
                ...
                if (array_key_exists_wrapper('destination', $content)) {
        ...
        foreach ($check as $key => $content) {
            $fileerrors  = array();
            if (array_key_exists_wrapper('validators', $content) && $content['validated']) {
                continue;
            }
    ...
    public function addFilters(array $filters, $files = null)
    {
        foreach ($filters as $key => $spec) {
            ...
            if (is_int($key)) {
                ...
                if (is_array($spec)) {
                    if (!array_key_exists_wrapper('filter', $spec)) {
                        continue;
                    }
    ...
    protected function _filter($files = null)
    {
        ...
        foreach ($check as $name => $content) {
            if (array_key_exists_wrapper('filters', $content)) {

...
class Zend_Ldap
{
    ...
    public function setOptions($options)
    {
        ...
        foreach ($permittedOptions as $key => $val) {
            if (array_key_exists_wrapper($key, $options)) {
                $val = $options[$key];
                unset($options[$key]);
    ...
    {
        if (array_key_exists_wrapper('dn', $entry)) unset($entry['dn']);
    ...
    public function add($dn, array $entry)
    {
        ...
        foreach ($rdnParts as $key => $value) {
            ...
            if (!array_key_exists_wrapper($key, $entry)) {
                $entry[$key] = array($value);
    ...
    public function update($dn, array $entry)
    {
        ...
        foreach ($rdnParts as $key => $value) {
            $value = Zend_Ldap_Dn::unescapeValue($value);
            if (array_key_exists_wrapper($key, $entry) && !in_array($value, $entry[$key])) {
                $entry[$key] = array_merge(array($value), $entry[$key]);
            }
        }

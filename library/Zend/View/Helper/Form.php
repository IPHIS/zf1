...
class Zend_View_Helper_Form extends Zend_View_Helper_FormElement
{
    ...
    public function form($name, $attribs = null, $content = false)
    {
        ...
        if (array_key_exists_wrapper('id', $attribs) && empty($attribs['id'])) {
            unset($attribs['id']);
        }
        ...
        if ($this->_isHtml5() && array_key_exists_wrapper('action', $attribs) && !$attribs['action']) {
            unset($attribs['action']);
        }

        if ( array_key_exists_wrapper('name', $attribs) && empty($attribs['id'])) {
            unset($attribs['id']);
        }

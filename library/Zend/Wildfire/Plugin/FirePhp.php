...
class Zend_Wildfire_Plugin_FirePhp implements Zend_Wildfire_Plugin_Interface
{
    ...
    {
      if (!array_key_exists_wrapper($key,$this->_options)) {
    ...
    protected function _recordMessage($structure, $data, $skipEncode=false)
    {
        switch($structure) {
            ...
            case self::STRUCTURE_URI_DUMP:
                ...
                if (!array_key_exists_wrapper('data',$data)) {
                    require_once 'Zend/Wildfire/Exception.php';
            ...
            case self::STRUCTURE_URI_FIREBUGCONSOLE:
                ...
                if (!isset($data['meta']) ||
                    !is_array($data['meta']) ||
                    !array_key_exists_wrapper('Type',$data['meta'])) {
        ...
        if (is_object($object)) {
            ...
            foreach ($properties as $just_name => $property) {
                ...
                if (!(isset($this->_objectFilters[$class])
                    ...
                    if (array_key_exists_wrapper($raw_name,$members)
                        && !$property->isStatic()) {
            ...
            foreach ($object as $key => $val) {
              ...
              if ($key=='GLOBALS'
                  && is_array($val)
                  && array_key_exists_wrapper('GLOBALS',$val)) {

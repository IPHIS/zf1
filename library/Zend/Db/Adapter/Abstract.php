...
abstract class Zend_Db_Adapter_Abstract
{
    ...
    public function __construct($config)
    {
         ...
         */
        if (array_key_exists_wrapper('options', $config)) {
            // can't use array_merge() because keys might be integers
            foreach ((array) $config['options'] as $key => $value) {
                $options[$key] = $value;
            }
        }
        if (array_key_exists_wrapper('driver_options', $config)) {
        ...
        if (array_key_exists_wrapper(Zend_Db::CASE_FOLDING, $options)) {
        ...
        if (array_key_exists_wrapper(Zend_Db::FETCH_MODE, $options)) {
        ...
        if (array_key_exists_wrapper(Zend_Db::AUTO_QUOTE_IDENTIFIERS, $options)) {
        ...
        if (array_key_exists_wrapper(Zend_Db::ALLOW_SERIALIZATION, $options)) {
        ...
        if (array_key_exists_wrapper(Zend_Db::AUTO_RECONNECT_ON_UNSERIALIZE, $options)) {
            $this->_autoReconnectOnUnserialize = (bool) $options[Zend_Db::AUTO_RECONNECT_ON_UNSERIALIZE];
        }

        // create a profiler object
        $profiler = false;
        if (array_key_exists_wrapper(Zend_Db::PROFILER, $this->_config)) {
    ...
    protected function _checkRequiredOptions(array $config)
    {
        ...
        if (! array_key_exists_wrapper('password', $config)) {
            /**
             * @see Zend_Db_Adapter_Exception
             */
            require_once 'Zend/Db/Adapter/Exception.php';
        ...
        if (! array_key_exists_wrapper('username', $config)) {
            /**
             * @see Zend_Db_Adapter_Exception
             */
            require_once 'Zend/Db/Adapter/Exception.php';
    ...
    public function quote($value, $type = null)
    {
        ...
        if ($type !== null && array_key_exists_wrapper($type = strtoupper($type), $this->_numericDataTypes)) {
            $quotedValue = '0';

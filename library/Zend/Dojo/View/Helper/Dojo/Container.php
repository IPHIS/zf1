...
class Zend_Dojo_View_Helper_Dojo_Container
{
    ...
    public function removeLayer($path)
    {
        ...
        if (array_key_exists_wrapper($path, $layers)) {
            unset($layers[$path]);
    ...
    {
        $option = (string) $option;
        if (array_key_exists_wrapper($option, $this->_djConfig)) {
    ...
    {
        if (array_key_exists_wrapper($id, $this->_dijits)) {
            require_once 'Zend/Dojo/View/Exception.php';
    ...
    {
        return array_key_exists_wrapper($id, $this->_dijits);
    }

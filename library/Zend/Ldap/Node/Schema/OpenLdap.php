...
class Zend_Ldap_Node_Schema_OpenLdap extends Zend_Ldap_Node_Schema
{
    ...
    protected function _parseAttributeType($value)
    {
        ...
        if (array_key_exists_wrapper('syntax', $attributeType)) {
    ...
    protected function _resolveInheritance(Zend_Ldap_Node_Schema_Item $node, array $repository)
    {
        ...
        foreach ($parents as $parent) {
            if (!array_key_exists_wrapper($parent, $repository)) continue;
            if (!array_key_exists_wrapper('_parents', $data) || !is_array($data['_parents'])) {
               $data['_parents'] = array();
           }
    ...
    {
        if (!array_key_exists_wrapper('name', $data) || empty($data['name'])) {
            // force a name
            $data['name'] = $data['oid'];
        }

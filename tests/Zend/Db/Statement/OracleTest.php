...
class Zend_Db_Statement_OracleTest extends Zend_Db_Statement_TestCommon
{
    ...
    public function testStatementReturnNullWithEmptyField()
    {
        ...
        $result = $this->_db->fetchAll($select);
        $this->assertTrue(array_key_exists_wrapper('product_name', $result[0]), 'fetchAll must return null for empty fields with Oracle');
        $result = $this->_db->fetchRow($select);
        $this->assertTrue(array_key_exists_wrapper('product_name', $result), 'fetchRow must return null for empty fields with Oracle');
    }
